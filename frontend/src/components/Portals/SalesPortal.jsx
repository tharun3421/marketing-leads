    ...
    return (
      <div className="fixed inset-0 bg-slate-950/45 backdrop-blur-xs flex items-center justify-center p-4 z-150 overflow-y-auto">
        <motion.div
          ...
          className="bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-850 rounded-2xl w-full max-w-[92vw] xl:max-w-7xl max-h-[88vh] flex flex-col shadow-2xl overflow-hidden"
        >
          {/* Modal Header */}
          ...
          <div className="p-6 overflow-y-auto flex-1">
            {list.length === 0 ? (
              ...
              <div className="overflow-x-auto border border-gray-100 dark:border-slate-800/60 rounded-xl">
                <table className="w-full min-w-[1000px] text-left text-sm border-collapse table-fixed">
                  <colgroup>
                    <col className="w-[9%]" />
                    <col className="w-[17%]" />
                    <col className="w-[11%]" />
                          ...
                          {(() => {
                            ...
                            if (perTeam && perTeam.length > 0 && (client.workflowStatus === 'In Progress' || client.workflowStatus === 'Completed' || client.workflowStatus === 'Allocated')) {
                              ...
                              return (
                                <div className="flex flex-col gap-1">
                                  {perTeam.map((t, i) => (
                                    <span key={i} className={`px-2 py-0.5 rounded-md text-[10px] font-bold whitespace-nowrap w-fit ${statusColor(t.status)}`}>
                                      {t.label}: {t.status}
                                    </span>
                                  ))}
                                </div>
                        ...
                        <td className="p-3 text-xs">
                          {(() => {
                            ...
                            return (
                              <div className="flex flex-col gap-1">
                                {assignees.map((a, i) => (
                                  <span key={i} className={`px-2 py-0.5 rounded-md text-[10px] font-bold whitespace-nowrap w-fit ${a.color}`}>
                                    {a.team}: {a.name || <span className="opacity-50 italic">Unclaimed</span>}
                                  </span>
                                ))}
                              </div>
