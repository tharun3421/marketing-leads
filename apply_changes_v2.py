...
if current_view_state in code:
    ...
    print("Warning: Could not find currentView state variable.")
...
if assign_tasks_start_idx != -1:
    ...
    print("Removed Assign Tasks block and injected helpers.")
else:
    print("Warning: Could not find assign-tasks block.")

# 3. Remove Logout button from top header
# We will do a robust regex to find the button
logout_btn_match = re.search(r"<Button\s+variant=\"outline\"\s+size=\"sm\"\s+onClick=\{\(\)\s*=>\s*\{\s*logout\(\);\s*onAddToast\('Sign Out Success',\s*'Admin session terminated.',\s*'info'\);\s*\}\}\s+icon=\{LogOut\}\s*>\s*Sign Out Admin\s*</Button>", code)
if logout_btn_match:
    code = code[:logout_btn_match.start()] + code[logout_btn_match.end():]
    print("Removed logout button from top header.")
else:
    # Try another search if whitespace differs
    ...
    if simple_logout_marker in code:
        ...
        print("Warning: Could not find top header Logout Button.")
...
if metrics_idx != -1:
    ...
    print("Warning: Could not find 6-Card Metrics Row.")
...
if tbody_start != -1:
    ...
    print("Warning: Could not find Central Clients Table tbody.")
...
# 8. Main Roster Grid (Sales & Tech Directories side-by-side, config sidebar on right)
...
if grid_start_idx != -1:
    ...
    new_grid_code = """{/* Main Roster Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-indigo-650 transition-colors" />
                      </div>
                    </div>
        ...
        {/* Configuration Stack (Register Staff Member & Google Sheets Configuration) */}
        <div className="lg:col-span-1 space-y-6">
          ...
          {/* Admin Logout Button stacked at the bottom of the sidebar */}
    ...
    print("Re-placed Main Roster Grid.")
else:
    print("Warning: Could not find {/* Main Roster Grid */} comment.")
...
"""
    ...
    print("Injected modals successfully.")
else:
    print("Warning: Could not find delete representative confirmation modal comment.")
