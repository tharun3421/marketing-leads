...
print("Created backup of AdminPortal.jsx")
...
if current_view_state in code:
    ...
    print("Warning: Could not find currentView state variable to inject new states.")

# 2. Extract Helpers from the step text files
...
# 3. Replace the 'assign-tasks' block (lines 1135 to 1788) with the helper functions
...
if assign_tasks_start_idx != -1:
    return_stmt_idx = code.find("return (", assign_tasks_start_idx)
    assign_tasks_end_idx = code.rfind("}", assign_tasks_start_idx, return_stmt_idx) + 1
    
    code = code[:assign_tasks_start_idx] + helpers_code + code[assign_tasks_end_idx:]
    print("Removed Assign Tasks block and injected helpers successfully.")
else:
    print("Warning: Could not find assign-tasks view block to replace.")

# 4. Remove Logout Button from the Header
logout_btn_pattern = r"<Button\s+variant=\"outline\"\s+size=\"sm\"\s+onClick=\{\(\)\s*=>\s*\{\s*logout\(\);\s*onAddToast\('Sign Out Success',\s*'Admin session terminated.',\s*'info'\);\s*\}\}\s+icon=\{LogOut\}\s*>\s*Sign Out Admin\s*</Button>"
code, count = re.subn(logout_btn_pattern, "", code)
if count > 0:
    print("Removed logout button from top header.")
else:
    # Try a simpler replace if whitespace differed
    simple_logout_marker = "logout();\n              onAddToast('Sign Out Success', 'Admin session terminated.', 'info');\n            }}\n            icon={LogOut}\n          >\n            Sign Out Admin\n          </Button>"
    if simple_logout_marker in code:
        # Find the preceding '<Button'
        btn_start = code.rfind("<Button", 0, code.find(simple_logout_marker))
        btn_end = code.find("</Button>", btn_start) + len("</Button>")
        code = code[:btn_start] + code[btn_end:]
        print("Removed logout button from top header via fallback check.")
    else:
        print("Warning: Could not find top header Logout Button to remove.")

# 5. Replace 6-Card Metrics Row with Clickable Cards
metrics_marker = "{/* 6-Card Metrics Row */}"
metrics_idx = code.find(metrics_marker)
if metrics_idx != -1:
    # Find the closing </div> of this grid container. The grid container starts right below the marker.
    grid_start_idx = code.find("<div className=", metrics_idx)
    # Scan characters to find matching closing div
    open_divs = 0
    i = grid_start_idx
    while i < len(code):
        if code[i:i+4] == "<div":
    ...
    clickable_metrics_code = """{/* 6-Card Metrics Row */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        >
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Allocated</p>
            <p className="text-xl font-bold text-gray-900 dark:text-white mt-0.5">
        ...
        >
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Claimed Tasks</p>
            <p className="text-xl font-bold text-gray-900 dark:text-white mt-0.5">
        ...
        >
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">In Progress</p>
            <p className="text-xl font-bold text-gray-900 dark:text-white mt-0.5">
    ...
    print("Replaced 6-Card Metrics row with clickable versions.")
else:
    print("Warning: Could not find 6-Card Metrics Row comment.")

# 6. Central Clients Table clickable ID and Created By Column
...
if central_clients_thead_old in code:
    ...
    if central_clients_thead_old_r in code:
        ...
        print("Warning: Could not find Central Clients Table header columns.")

# Table Row mapping replacement
old_row_block = """                  <tr key={client._id} className="hover:bg-indigo-500/3 dark:hover:bg-indigo-500/1 transition-colors">
                    ...
                    <td className="p-3 text-gray-900 dark:text-white font-bold">
                      ...
                      {client.companyName && (
                        <div className="text-xs text-gray-400 dark:text-gray-550 font-normal mt-0.5">
                          {client.companyName}
                        </div>
                      )}
                    </td>
...
new_row_block = """                  <tr key={client._id} className="hover:bg-indigo-500/3 dark:hover:bg-indigo-500/1 transition-colors">
                    <td className="p-3">
                      <button
                        onClick={() => {
...
if old_row_block in code:
    code = code.replace(old_row_block, new_row_block)
    print("Replaced Central Clients Table row mapper block.")
else:
    old_row_block_r = old_row_block.replace("\n", "\r\n")
    new_row_block_r = new_row_block.replace("\n", "\r\n")
    if old_row_block_r in code:
        code = code.replace(old_row_block_r, new_row_block_r)
        print("Replaced Central Clients Table row mapper block (CRLF).")
    else:
        print("Warning: Could not find Central Clients Table row mapper block.")

# 7. Make Client ID column & clickable button in Inspected Leads table (portfolio view)
...
if portfolio_thead_old in code:
    ...
    print("Injected Client ID header into Inspected Leads table.")
else:
    ...
    if portfolio_thead_old_r in code:
        ...
        print("Injected Client ID header into Inspected Leads table (CRLF).")
    else:
        print("Warning: Could not find Inspected Leads Table header.")
...
if portfolio_row_old in code:
    ...
    print("Injected Client ID clickable cell into Inspected Leads table rows.")
else:
    ...
    if portfolio_row_old_r in code:
        ...
        print("Injected Client ID clickable cell into Inspected Leads table rows (CRLF).")
    else:
        print("Warning: Could not find Inspected Leads Table row start.")

# 8. Re-order Main Roster Grid and Add Edit Pen Buttons
# Let's completely construct the Main Roster Grid block and replace it.
grid_marker = "{/* Main Roster Grid */}"
grid_start_idx = code.find(grid_marker)
if grid_start_idx == -1:
    print("Error: Could not find {/* Main Roster Grid */} comment.")
else:
    # Find the end of this roster grid block
    ...
    grid_end_idx = code.find("</div>", grid_end_idx) + 6 # include the closing </div>
    
    new_grid_code = """{/* Main Roster Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Salesforce & Technical Directories Side by Side */}
                        ...
                        {rep.id && (
                          <>
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg hover:bg-indigo-500/10 text-indigo-500 hover:text-indigo-605 transition-colors cursor-pointer opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:focus:opacity-100"
                          ...
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <h4 className="font-bold text-gray-900 dark:text-white flex flex-wrap items-center gap-1.5">
                                {tech.name}
                                <span className="text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-655 dark:text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
                                  {tech.team === 'design' ? 'Designing' : tech.team === 'developer' ? 'Developer' : tech.team === 'ads' ? 'Ads' : 'General'}
                                </span>
                              </h4>
                              <span className="text-[10px] text-gray-450 dark:text-gray-550 font-semibold">
                                @{tech.username}
                              </span>
                            </div>
                          </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {/* Admin Logout Option - Stacked at the bottom of the sidebar */}
          <Card className="border border-red-500/10 dark:border-red-500/5">
            <Button
              variant="outline"
              className="w-full !text-red-500 hover:!bg-red-50 dark:hover:!bg-red-950/20 !border-red-200 dark:!border-red-900/30 flex items-center justify-center gap-2 cursor-pointer transition-all"
              onClick={() => {
                logout();
                onAddToast('Sign Out Success', 'Admin session terminated.', 'info');
              }}
    ...
    code = code[:grid_start_idx] + new_grid_code + code[grid_end_idx:]
    print("Re-ordered Main Roster Grid with side-by-side layout, scroll containment, Edit buttons, and Stacked Sidebar + Logout.")

# 9. Append Metrics list, Detailed Client profile, and Edit Employee modals
delete_modal_comment = "{/* Delete Representative Confirmation Modal */}"
delete_idx = code.find(delete_modal_comment)
if delete_idx != -1:
    # Read step5741 modals code
    ...
    # Extract the modals from step5741
    # Metrics List Modal and Detailed Client Profile Editor Modal start at line 3 and end before the final returns
    # Let's locate "{/* Metrics List Modal */}"
    metrics_modal_start = step5741.find("{/* Metrics List Modal */}")
    # Detailed Client Profile Editor Modal ends at line 608: "      })()}"
    profile_modal_end = step5741.find("      })()}", metrics_modal_start) + len("      })()}")
    
    modals_extracted = step5741[metrics_modal_start:profile_modal_end]
    
    # Define our custom Edit Employee Modal
    edit_employee_modal = """
      {/* Edit Employee Modal */}
      {employeeToEdit && (
...
"""
    ...
    all_modals_code = modals_extracted + "\n" + edit_employee_modal + "\n"
    code = code[:delete_idx] + all_modals_code + code[delete_idx:]
    print("Injected Modals into AdminPortal.jsx.")
else:
    print("Warning: Could not find delete representative modal comment to inject new modals.")

# Write back modified code
with open(file_path, "w", encoding="utf-8") as f:
    f.write(code)

print("Finished applying all modifications to AdminPortal.jsx")
